...
class TierlistApp {
  constructor() {
    ...
    this.tierlist = null;
    this.isOwner = false;
    this.draggedElement = null; // Élément actuellement en cours de glisser-déposer
  }
  ...
  async uploadAndSyncImage(file) {
    try {
      ...
      // Ajoute à la FIN de la liste _blank (id: 0)
  ...
  async removeImageAndSync(hash) {
    // Retire l'image de tous les rangs et de la zone _blank
  ...
  renderEditorTierlist() {
    ...
    order.forEach(tierId => {
      if (tierId === 0) return; // Ignorer le tier 0 (_blank)
